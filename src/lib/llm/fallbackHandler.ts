import type { Provider } from './config';

export interface FallbackConfig {
  enabled: boolean;
  maxAttempts: number;
  retryableErrors: string[];
  timeout: number;
}

export interface FallbackEvent {
  timestamp: Date;
  feature: string;
  primaryProvider: Provider;
  fallbackProvider: Provider;
  reason: string;
  primaryError?: string;
  fallbackSuccess: boolean;
}

export interface FallbackContext {
  feature: string;
  primaryProvider: Provider;
  fallbackProvider: Provider;
}

export interface FallbackStats {
  totalFallbacks: number;
  successRate: number;
  byFeature: Record<string, number>;
  byReason: Record<string, number>;
}

export interface FallbackHandlerOptions {
  now?: () => Date;
}

type FallbackError = Error & {
  statusCode?: number;
  code?: string;
  timeout?: boolean;
  response?: {
    status?: number;
  };
};

export const DEFAULT_FALLBACK_CONFIG: FallbackConfig = {
  enabled: true,
  maxAttempts: 2,
  retryableErrors: ['5xx', '429', 'timeout'],
  timeout: 30_000,
};

export class FallbackHandler {
  private readonly now: () => Date;
  private readonly events: FallbackEvent[] = [];
  private fallbackCounter = 0;

  constructor(
    private readonly config: FallbackConfig = DEFAULT_FALLBACK_CONFIG,
    options: FallbackHandlerOptions = {},
  ) {
    this.now = options.now ?? (() => new Date());
  }

  shouldFallback(error: unknown, _provider: Provider): boolean {
    if (!this.config.enabled) {
      return false;
    }

    const typedError = this.asFallbackError(error);
    const statusCode = this.getStatusCode(typedError);

    if (statusCode === 429) {
      return this.config.retryableErrors.includes('429');
    }

    if (statusCode !== null && statusCode >= 500 && statusCode < 600) {
      return this.config.retryableErrors.includes('5xx');
    }

    return this.isTimeoutError(typedError) && this.config.retryableErrors.includes('timeout');
  }

  async executeWithFallback<T>(
    primaryFn: () => Promise<T>,
    fallbackFn: () => Promise<T>,
    context: FallbackContext,
  ): Promise<T> {
    try {
      return await primaryFn();
    } catch (primaryError) {
      if (!this.shouldFallback(primaryError, context.primaryProvider)) {
        throw primaryError;
      }

      this.fallbackCounter += 1;
      const reason = this.classifyReason(primaryError);
      const primaryErrorMessage = primaryError instanceof Error ? primaryError.message : String(primaryError);

      try {
        const result = await fallbackFn();
        this.recordEvent({
          timestamp: this.now(),
          feature: context.feature,
          primaryProvider: context.primaryProvider,
          fallbackProvider: context.fallbackProvider,
          reason,
          primaryError: primaryErrorMessage,
          fallbackSuccess: true,
        });
        return result;
      } catch (fallbackError) {
        this.recordEvent({
          timestamp: this.now(),
          feature: context.feature,
          primaryProvider: context.primaryProvider,
          fallbackProvider: context.fallbackProvider,
          reason,
          primaryError: primaryErrorMessage,
          fallbackSuccess: false,
        });
        throw fallbackError;
      }
    }
  }

  async getFallbackStats(durationMs: number): Promise<FallbackStats> {
    const cutoffMs = this.now().getTime() - durationMs;
    const recentEvents = this.events.filter((event) => event.timestamp.getTime() >= cutoffMs);

    const byFeature: Record<string, number> = {};
    const byReason: Record<string, number> = {};

    for (const event of recentEvents) {
      byFeature[event.feature] = (byFeature[event.feature] ?? 0) + 1;
      byReason[event.reason] = (byReason[event.reason] ?? 0) + 1;
    }

    const successful = recentEvents.filter((event) => event.fallbackSuccess).length;

    return {
      totalFallbacks: recentEvents.length,
      successRate: recentEvents.length === 0 ? 0 : successful / recentEvents.length,
      byFeature,
      byReason,
    };
  }

  getFallbackCount(): number {
    return this.fallbackCounter;
  }

  getEvents(): FallbackEvent[] {
    return [...this.events];
  }

  private recordEvent(event: FallbackEvent): void {
    this.events.push(event);
  }

  private classifyReason(error: unknown): string {
    const typedError = this.asFallbackError(error);
    const statusCode = this.getStatusCode(typedError);

    if (statusCode === 429) {
      return '429';
    }

    if (statusCode !== null && statusCode >= 500 && statusCode < 600) {
      return '5xx';
    }

    if (this.isTimeoutError(typedError)) {
      return 'timeout';
    }

    return 'unknown';
  }

  private getStatusCode(error?: FallbackError): number | null {
    if (!error) {
      return null;
    }

    if (typeof error.statusCode === 'number') {
      return error.statusCode;
    }

    if (typeof error.response?.status === 'number') {
      return error.response.status;
    }

    return null;
  }

  private isTimeoutError(error?: FallbackError): boolean {
    if (!error) {
      return false;
    }

    return error.timeout === true
      || error.name === 'AbortError'
      || error.code === 'ETIMEDOUT'
      || /timeout/i.test(error.message);
  }

  private asFallbackError(error: unknown): FallbackError | undefined {
    return error instanceof Error ? error as FallbackError : undefined;
  }
}
